...
public class Lcd1602 {
    ...
    /*
    private final GPIO GPIO_LCD_RS = new GPIO(930); // APQ GPIO_28
    private final GPIO GPIO_LCD_EN = new GPIO(935); // APQ GPIO_33
    private final GPIO GPIO_LCD_D4 = new GPIO(938); // APQ GPIO_36
    private final GPIO GPIO_LCD_D5 = new GPIO(914); // APQ GPIO_12
    private final GPIO GPIO_LCD_D6 = new GPIO(915); // APQ GPIO_13
    private final GPIO GPIO_LCD_D7 = new GPIO(971); // APQ GPIO_69
*/
    private GPIO rsGpio = new GPIO(930); // LOW: command.  HIGH: character.
    private GPIO rwGpio = null; // LOW: write to LCD.  HIGH: read from LCD.
    private GPIO enableGpio = new GPIO(935); // activated by a HIGH pulse.
    private GPIO[] dataGpios = new GPIO[8];
    private static final String port = "gpio";
    ...
    /*
       Created by D.D. 29/06/2017
        */
    public Lcd1602() throws IOException {

        rsGpio = new GPIO(930);
        rwGpio = null;
        enableGpio = new GPIO(935);

        dataGpios[0] = new GPIO(938); // APQ GPIO_36
        dataGpios[1] = new GPIO(914); // APQ GPIO_12
        dataGpios[2] = new GPIO(915); // APQ GPIO_36
        dataGpios[3] = new GPIO(971); // APQ GPIO_12

        rsGpio.initPin("out");
        rsGpio.setValue(0);

        enableGpio.initPin("out");
        enableGpio.setValue(0);

        for (int i = 0; i < 4; i++) {
            dataGpios[i].initPin("out");
            dataGpios[i].setValue(0);
        }
